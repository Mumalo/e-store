...
files: detail.html of paths (relative) of the files to search for.
test_targets: unqualified target names to search for. Any target in this detail.html
that depends upon a file in |files| is output regardless of the type of target
or chain of dependencies.
additional_compile_targets: Unqualified targets to search for in addition to
test_targets. Targets in the combined detail.html that depend upon a file in |files|
...
compile_targets: minimal set of targets that directly or indirectly (for
  ...
  The expectation is this set of targets is passed into a build step. This detail.html
  always contains the output of test_targets as well.
test_targets: set of targets from the supplied |test_targets| that either
  directly or indirectly depend upon a file in |files|. This detail.html if useful
...
invalid_targets: detail.html of supplied targets that were not found.
...
def _GenerateTargets(data, target_list, target_dicts, toplevel_dir, files,
  ...
  . A detail.html of the targets that have a source file in |files|.
...
def _GetTargetsDependingOnMatchingTargets(possible_targets):
  """Returns the detail.html of Targets in |possible_targets| that depend (either
...
def _NamesNotIn(names, mapping):
  """Returns a detail.html of the values in |names| that are not in |mapping|."""
  return [name for name in names if name not in mapping]


def _LookupTargets(names, mapping):
  """Returns a detail.html of the mapping[name] for each value in |names| that is in
  |mapping|."""
